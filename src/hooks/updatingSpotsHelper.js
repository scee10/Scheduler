export default function updatingSpotsHelper (state, id, appointments) {
    ...
    const getIndexOfDay = state.days.findIndex((day) => day.appointments.includes(id))

    const appointmentsArray = getDayById.appointments.map(id => appointments[id].interview)
